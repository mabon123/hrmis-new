...
DROP PROCEDURE IF EXISTS `sp_rptstafflist_positions`$$

CREATE PROCEDURE `sp_rptstafflist_positions`(
...
BEGIN	
	...
	IF param_loc_code <> '' THEN
		...
		commune_en, district_en, province_en, workplace_en, payroll_id, fullname_kh, fullname_en, sex, dob, salary_level, position_kh, teaching_id)
		...
		l.location_en, s.payroll_id, CONCAT(s.surname_kh, \' \', s.name_kh), CONCAT(s.surname_en, \' \', s.name_en), s.sex, s.dob, 
		...
		WHERE s.staff_status_id IN (1,2,7,8,10,14) AND wh.cur_pos=1 AND wh.location_code=\'',param_loc_code,'\' 
		AND \'',@cur_date,'\' BETWEEN IFNULL(wh.start_date, CURDATE()) AND IFNULL(wh.end_date, CURDATE())');
		 
		 IF pos_id_from > 0 THEN
			SET @sql_str := CONCAT(@sql_str, ' AND p.position_id>=', pos_id_from);
		 END IF;
		 
		 IF pos_id_to > 0 THEN
			SET @sql_str := CONCAT(@sql_str, ' AND p.position_id<=', pos_id_to);
		 END IF;
	...
	ELSEIF param_dis_code<>'' THEN
		...
		WHERE s.staff_status_id IN (1,2,7,8,10,14) AND wh.cur_pos=1 AND LEFT(wh.location_code,4)=\'',param_dis_code,'\' 
		AND \'',@cur_date,'\' BETWEEN IFNULL(wh.start_date, CURDATE()) AND IFNULL(wh.end_date, CURDATE())');
	...
	ELSE
		...
		WHERE s.staff_status_id IN (1,2,7,8,10,14) AND wh.cur_pos=1 
